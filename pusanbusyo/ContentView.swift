...
struct ClusteredMapView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> MKMapView {
        ...
        // ⛳️⛳️ App Review 전용: 시작 위치를 '부산시청'으로 강제 고정
           // ▶▶▶ REVIEW_MODE_START: FIXED START AT BUSAN CITY HALL (Yeonje-gu)
           // let lm = CLLocationManager()
           // let startCenter: CLLocationCoordinate2D =
           //     lm.location?.coordinate ?? CLLocationCoordinate2D(latitude: 36.351, longitude: 127.385) // (구) 대전 기본
           let startCenter = CLLocationCoordinate2D(latitude: 35.17955, longitude: 129.07564)
           // ◀◀◀ REVIEW_MODE_END
           // (원복하려면 위 고정값을 주석 처리하고 아래 3줄의 lm/location 사용을 해제)
        ...
        ZStack {
            ClusteredMapView(vm: vm, recenterRequest: $recenterRequest)
                ...
                .task {
                    ...
                    // ⛳️⛳️ App Review 전용: 초기 데이터 로딩도 '부산시청' 기준으로
                            // ▶▶▶ REVIEW_MODE_START: FIXED RELOAD CENTER (BUSAN CITY HALL)
                            // await vm.reload(center: .init(latitude: 36.351, longitude: 127.385)) // (구) 대전
                            await vm.reload(center: .init(latitude: 35.17955, longitude: 129.07564))
                            // ◀◀◀ REVIEW_MODE_END
                            // (원복: 부산 좌표 줄을 주석 처리하고 대전/혹은 '현재 위치' 로직으로 대체)
                }
